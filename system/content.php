...
$path = 'posts';
//if($category == 'post' || $category == 'site' || $category == 'tag' || $category == 'rss') $path = 'posts';
if ($category == 'page') {
    $path = 'pages';
}

$content = preg_replace('/\[meta\](.*?)\[\/meta\]/s', '', file_get_contents('./' . $path . '/' . $post . $fileExt));
if (strstr(file_get_contents('./' . $path . '/' . $post . $fileExt), '[meta]') && strstr(file_get_contents('./' . $path . '/' . $post . $fileExt), '[/meta]')) {
    $meta_content = preg_match_all('/\[meta\](.*?)\[\/meta\]/s', file_get_contents('./' . $path . '/' . $post . $fileExt), $meta);
    ...
    if (count($meta_date) != 0) {
        ...
        $post_date = date($date, filemtime('./' . $path . '/' . $post . $fileExt));
    }
