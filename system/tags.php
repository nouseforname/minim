...
if (substr($_SERVER['QUERY_STRING'], 0, 4) === 'tag-') {
    $category = 'tag';
    include './system/posts.php';
    $tag = substr(urldecode($_SERVER['QUERY_STRING']), 4);
    if (preg_match('/tag-[0-9]-*/', $_SERVER['QUERY_STRING'])) {
    ...
    for ($i = count($posts) - 1; $i > 0; $i--) {
        $post = $posts[$i];
        if (strstr(file_get_contents('./posts/' . $post . $fileExt), '[meta]') && strstr(file_get_contents('./posts/' . $post . $fileExt), '[/meta]')) {
            include './system/content.php';
            if (in_array($tag, $tags_link)) {
                $tag_found   = true;
                $post_number = array_search($post . 'md', $posts);
